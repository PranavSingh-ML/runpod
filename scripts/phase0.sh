#!/usr/bin/env bash
# Phase 0 - runs FROM THE LAPTOP, drives a pod created from the imgedit image under test (v0, v3, ...) with PHASE0=1.
...
echo "== environment =="
ENV='set -a; . <(tr "\0" "\n" < /proc/1/environ | grep -E "^(PATH|HF_|PYTORCH|API_TOKEN|MODEL|LORA|QUANT|PE_|PIPELINE|DEFAULT_|MAX_|MIN_|TEXT_ENCODER|TOKENIZERS)="); set +a;'
...
echo "== probe (downloads the weights on first run: v3 ~33GB model + ~19GB rewriter, v2 58GB; 5-15 min) =="
...
scp -i "$KEY" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -P "$PORT" \
  "root@$HOST:/app/{requirements.lock.txt,phase0_report.json,phase0_out.png,phase0_out_2.png,phase0_out_3_rewritten.png}" phase0/ || true
ls -la phase0/
echo
